...
angular.module('yetienceApp', ['ui.router', 'formly', 'formlyBootstrap', 'jsonFormatter', 'ui.bootstrap', 'angular-md5', 'ngCookies', 'LocalStorageModule','angularTrix','ngTagsInput'])
    ...
    .controller('ListController', ['$scope', 'SettingsService', '$state', '$rootScope', 'UtilsService', 'CommService', 'UpgradeService', '$location', 'localStorageService', '$modal', 'configurationFields', '$timeout', 'FeatureStatusService', 'WidgetUpdate', function($scope, SettingsService, $state, $rootScope, UtilsService, CommService, UpgradeService, $location, localStorageService, $modal, configurationFields, $timeout, FeatureStatusService, WidgetUpdate) {
        ...
        $rootScope.$on('setup_initialized', function() {
            if (SettingsService.setup().widgets && SettingsService.setup().widgets.length == 0) {
                ...
                //CommService.createNewEvent('list_displayed', { 'selected_theme': SettingsService.setup().widgets[0].theme });
        ...
        $scope.clickedRecommends = function(item) {
            CommService.createNewEvent('d_recommend_click_' + item.label, '')
            $timeout(function() {
        ...
        $scope.yelShowAffiliate = function() {
            //CommService.createNewEvent('d_affiliate_interested','')
            $timeout(function() {
        ...
        $scope.addNewWidget = function() {
            CommService.createNewEvent('d_add_another_widget', '')
                /*
        ...
        $scope.sendFeedback = function(feedback, closed) {
            ...
            CommService.createNewEvent('d_feedback', {
        ...
        */
        ...
        $scope.reviewClicked = function() {
            ...
            CommService.updateReview('YES')
                .then(function() {
                    CommService.createNewEvent('d_clicked_review', '')
                })
        }
        ...
        $scope.reviewClosed = function() {
            ...
            CommService.updateReview('CLOSED')
                .then(function() {
                    //CommService.createNewEvent('d_closed_review','')
                })
        }
...
angular.module('yetienceApp')
    .controller('businessGoalsController', ['$scope', '$state', '$rootScope', 'SettingsService', 'CommService', 'WidgetUpdate', function($scope, $state, $rootScope, SettingsService, CommService, WidgetUpdate) {
        ...
        $scope.selectGoal = function(goal) {
            ...
            // CommService.createNewEvent("goal_selected", {
            //     'selected_goal': label
            // });
...
angular.module('yetienceApp')
    .controller('widgetTypeController', ['$scope', '$state', '$rootScope', 'SettingsService', 'CommService', 'WidgetUpdate', function($scope, $state, $rootScope, SettingsService, CommService, WidgetUpdate) {
        ...
        $scope.selectPlacement = function(placement) {
            ...
            // CommService.createNewEvent("placement_selected", {
            //     'selected_placement': label
            // });
    ...
    .controller('buildSelectController', ['$scope', 'SettingsService', '$state', '$rootScope', 'CommService', 'UtilsService', 'WidgetUpdate', '$modal', 'UpgradeService', function($scope, SettingsService, $state, $rootScope, CommService, UtilsService, WidgetUpdate, $modal, UpgradeService) {
        ...
        $scope.selectTheme = function(label, theme) {
            ...
            if (label == 'blank-popup' && !SettingsService.hasFeature('customHtml')) {
                ...
                CommService.createNewEvent("d_HTML_popup_selected_free", "");
                CommService.createNewEvent("d_seen_premium","")
            ...
            CommService.createNewEvent("d_theme_selected", {
            ...
            // if (theme.categories[0] == 'subscribe') {
...
angular.module('yetienceApp')
    .controller('buildIntegrateController', ['$scope', '$rootScope', 'CommService', 'UtilsService', 'WidgetUpdate','SettingsService', function($scope, $rootScope, CommService, UtilsService, WidgetUpdate,SettingsService) {
        ...
        $scope.enableWelcomeEmail = function() {
            ...
            WidgetUpdate.sendEmail = true
            $scope.widget.components.welcomeEmail.values.sendWelcomeEmail = true
            CommService.createNewEvent("autoresponse_yes_clicked");
        }

        $scope.skipToNext = function(widget, customer) {
            CommService.createNewEvent("autoresponse_no_clicked");
        ...
        if(SettingsService.hasFeature('emailSignature')){
            $scope.widget.components.welcomeEmail.values.removeSignatureFeature = true
        } 
        
        } else {
            $scope.notEmailTheme = true
            console.log('not email')
        }


        //watch the list value, if exists -> enable the save popup button
        // $scope.$watch('widget.components.emailSubscription.values.list', function(newValue, oldValue) {
        //     if ((sent_events.indexOf('list_selected') < 0)) {
        //         if (newValue) {
        //             CommService.createNewEvent("list_selected");
        //             sent_events.push('list_selected')
        //             $rootScope.fetched_lists = true
        //         } else {
        //             $rootScope.fetched_lists = false
        //         }
        //     }
        // });

        //NOTE:
        //if fetched_lists and from_email_defined both are true -> enable save popup in build controller.js
        ...
        // $scope.$watch('widget.components.emailSubscription.values.askName', function(new_val, old_val) {
        //     if ((sent_events.indexOf('askName_enabled') < 0) && ($rootScope.fetched_lists == true)) {
        //         if (new_val == true) {
        //             // console.log('new is ' + new_val)
        //             sent_events.push('askName_enabled')
        //             CommService.createNewEvent("askName_enabled");
        //         } else {
        //             // console.log('new is ' + new_val)
        //         }
        //     }
        ...
        // $scope.$watch('widget.components.emailSubscription.values.thankyou', function(new_val, old_val) {
        //     if ($rootScope.fetched_lists == true) {
        //         sent_events.push('thankyou_page_selected')
        //         CommService.createNewEvent("thankyou_page_selected", {
        //             'thankyou_page': new_val
        //         });
        //     }
        // })
        ...
        $scope.$watch('widget.components.welcomeEmail.values.sendWelcomeEmail', function(new_val, old_val) {
            if ($scope.showEmail == true) {
                if (new_val == true) {
                    // console.log('new is ' + new_val)
                    CommService.createNewEvent("send_welcome_email_enabled");
                } else {
                    CommService.createNewEvent("send_welcome_email_disabled");
                    // console.log('new is ' + new_val)
                }
            }
        })
            ...
            if ($scope.showEmail == true) {
                if ((sent_events.indexOf('email_from_filled') < 0) && ($rootScope.fetched_lists == true)) {
                    if (new_val) {
                        ...
                        CommService.createNewEvent("email_from_filled");
                    }
                }
            }
            ...
            if ($scope.showEmail == true) {
                if ((sent_events.indexOf('email_subject_filled') < 0) && ($rootScope.fetched_lists == true)) {
                    if (new_val) {
                        ...
                        CommService.createNewEvent("email_subject_filled");
                        email_subject_sent = true
                    } else {
                        CommService.createNewEvent("email_subject_empty");
                    }
                }
            }
        })
        ...
        $scope.$watch('widget.components.welcomeEmail.values.ccEnabled', function(new_val, old_val) {
            if ($scope.showEmail == true) {
                if (new_val == true) {
                    // console.log('new is ' + new_val)
                    CommService.createNewEvent("cc_enabled");
                } else {
                    CommService.createNewEvent("cc_disabled");
                    // console.log('new is ' + new_val)
                }
            }
        })
        $scope.$watch('widget.components.welcomeEmail.values.ccEmail', function(new_val, old_val) {
            if ($scope.showEmail == true && (sent_events.indexOf('email_cc_filled') < 0)) {
                if (new_val) {
                    ...
                    // console.log('new is ' + new_val)
                    CommService.createNewEvent("email_cc_filled");
                } else {
                    CommService.createNewEvent("email_cc_filled");
                    // console.log('new is ' + new_val)
                }
        ...
        $scope.emailFocussed = function() {
            if ($scope.showEmail == true) {
                if ((sent_events.indexOf('email_content_clicked') < 0) && ($rootScope.fetched_lists == true)) {
                    ...
                    CommService.createNewEvent("email_content_clicked");
                }
            }
        }
    }])
...
angular.module('yetienceApp')
    .controller('extensionSaleController', ['$scope', 'CommService', 'SettingsService', '$rootScope', 'localStorageService', function($scope, CommService, SettingsService, $rootScope, localStorageService) {
        ...
        //CommService.createNewEvent("d_interested_premium", '');
...
*/
 ...
 angular.module('yetienceApp')
     ...
     .controller('affiliateController', ['$scope', '$rootScope', '$timeout', 'SettingsService', 'CommService',  '$timeout', function($scope, $rootScope, $timeout, SettingsService, CommService, $timeout) {
         ...
         //CommService.createNewEvent("d_interested_affiliate",'');

         $scope.registerNewAffiliate = function(affiliate) {
             CommService.createNewEvent('d_affiliate_registered')
                 $timeout(function() {
        ...
        return {
            controller: ['$scope', '$element', 'CommService', '$modal', '$stateParams', '$rootScope', 'SettingsService', 'UtilsService', 'WidgetUpdate', function($scope, $element, CommService, $modal, $stateParams, $rootScope, SettingsService, UtilsService, WidgetUpdate) {
                ...
                $scope.$watch('R.widget.components.namePlaceholder.values.text', function(newValue, oldValue, scope) {
                    ...
                    if($scope.mode == 'edit'){
                        // console.log('in edit mode')s
                        // $rootScope.widget.components.namePlaceholder.values.text = newValue
                    }
                });
            ...
            controller: ['$rootScope', '$element', '$scope', 'SettingsService', 'CommService', 'UtilsService', function($rootScope, $element, $scope, SettingsService, CommService, UtilsService) {
                    ...
                    if ((provider_is_premium == true) && !SettingsService.hasFeature('premiumEmail')) {
                        ...
                        if ($scope.provider && $scope.provider == 'ActiveCampaign') {
                            ...
                            window.open(generate_auth_url[$scope.provider](), "Mailchimp",
                                ...
                                //CommService.createNewEvent("d_email_provider", { 'provider': $scope.provider })
                            pollForLists()
                        }
                ...
                $scope.$watch('provider', function(new_val, old_val) {
                    if (new_val) {
                        ...
                        CommService.createNewEvent("provider_selected", { 'provider': new_val })
                ...
                function fetchAndAssignLists(provider) {
                    ...
                    if (provider && provider.indexOf('Yeloni') < 0 && provider.indexOf('Sendy') < 0) {
                ...
                //template: "LR{{list_requested}} FL {{fetched_lists}} <div ng-hide='!list_requested || fetched_lists'><button type='button' class='btn btn-primary'  ng-click='openPopup()'><i class='glyphicon glyphicon-envelope'></i> Connect Yeloni to your {{provider}} Account</button><br><small class='text-muted'>This opens a new window where you can login to your {{provider}} Account</small></div>",
...
angular.module('yetienceApp')
    .service('Component', ['$rootScope', function($rootScope) {
        ...
        var updater = {
            ...
            text: function(el, value, values, tag, widget) {
                //Get the element ID and set its placeholder value
                document.getElementById(el.context.id).placeholder = value
            },
            showName: function(el, value, values, tag, widget) {
                //Get the element ID and set its placeholder value
                var display = 'none'
                if(value == true){
                    display = ''
                }
                document.getElementById(el.context.id).style.display = display
            },
...
angular.module('yetienceApp')
    .service('CommService', ['$http', '$q', '$rootScope', 'md5', function($http, $q, $rootScope, md5) {
        ...
        function getReferrer() {
            ...
            if (yetience.referrer_id == '') {
                // console.log('referrer_id is null')
                D.resolve(null)
            } else {
                // console.log('referrer_id found')
                D.resolve(yetience.referrer_id)
            }
            return D.promise
        }
        ...
        function comm(url, method, transform, data, base) {
            ...
            $http[method](base + url, data)
                .then(function(res) {
                    ...
                    //C.createNewEvent('http_error', error_to_send)
                    D.reject()
                })
            return D.promise
        }
...
angular.module('yetienceApp')
    .service('UtilsService', ['$modal', '$state', function($modal, $state) {
        ...
        this.nestedValue = function(obj, fields) {
...
angular.module('yetienceApp')
    .service('WidgetUpdate', ['CommService', '$state', '$timeout', 'SettingsService', 'UtilsService', '$rootScope', '$modal', '$q', function(CommService, $state, $timeout, SettingsService, UtilsService, $rootScope, $modal, $q) {
        ...
        /*
        Each tab is depicted as a separate state
        The attributes can be values, or functions which take widget as input
        . If they are values, they are used as it is. Otherwise function is computed
        Attributes-
        show - Should the next button be shown on this state (boolean)
        tabTitle - Title of the current tab (string)
        create: Should this tab be shown while creating the widget
        edit: Should this tab be shown while editing the widget
        saveOnCompletion: After completing this step, is the next step saving the widget?
        */
        var states = {
            select: {
                show: false,
        ...
        function checkAndAskWidgetName(widget) {
            var D = $q.defer()
            var widget_name = UtilsService.nestedValue(widget, ['configuration', 'what', 'title'])
            if (angular.isString(widget_name)) {
                //title is already given
                D.resolve()
            } else {
                //open a dialog and ask for name
                ...
                var modalInstance = $modal.open({
                    ...
                    controllerAs: 'nameModal',
                    templateUrl: yetience.adminPath + '/src/partials/widget.name.modal.html',
                    keyboard: false
                })
            }
    ...
    .service('FeatureStatusService', ['$rootScope', function($rootScope, SettingsService) {
        ...
        this.emailSignature = function(widget, feature) {
            ...
            return {
                ...
                upgrade: "Remove 'Sent using Yeloni' from Autoresponder Emails"
            };
        }
        ...
        this.limitByReferrer = function(widget, feature) {
            ...
            return {
                ...
                upgrade: 'Show/hide widget to visitors from specific websites'
            };
        }
...
angular.module('yetienceApp')
    .constant('configurationFields', {
        ...
        where: [{
            ...
            key: "where_url_contains",
            type: "multiCheckbox",
            hideExpression: "(model.where.showOrHide != 'show' || model.where.other != 'specific' || model.where.specific.selector != 'url' )",
            templateOptions: {
                label: "Show popup when the url contains ",
                options: [],
                valueProp: 'ID',
            ...
            key: "where_url_contains",
            type: "multiCheckbox",
            hideExpression: "(model.where.showOrHide != 'hide' || model.where.other != 'specific' || model.where.hideOn.hideselector != 'url' )",
            templateOptions: {
                label: "Hide popup when the url contains ",
                options: [],
                valueProp: 'ID',
                labelProp: 'post_title'
            }
        ...
        whom: [{
            ...
            template: "<hr /><div><strong>Hide/Show widgets based on referrer (Premium)</strong></div>"
        },{
            type: "radio",
            key: "whom.limitByReferrerSetting",
            templateOptions: {
                label: " ",
                labelProp: "title",
                valueProp: "value",
                options: [{
                    title: "Show the popup/widget ONLY when the user arrives from the following websites",
                    value: "show"
                }, {
                    title: "Hide the popup/widget ONLY when the user arrives from the following websites",
                    value: "hide"
                }]
            },
            expressionProperties: {
                'templateOptions.disabled': "(model.advancedConfiguration != true && model.limitByReferrer != true )"
            }
