...
class Google
{
    ...
    private function handleEventData( Entities\Appointment $appointment )
    {
        ...
        foreach ( $appointment->getCustomerAppointments() as $ca ) {
            ...
            if ( $ca->get( 'extras' ) != '[]' ) {
                $extras = implode( ', ', array_map( function ( $extra ) { return $extra->get( 'title' ); }, apply_filters( 'bookly_service_extras_find_by_ids', array(), array_keys( json_decode( $ca->get( 'extras' ), true ) ) ) ) );
                if ( ! empty( $extras ) ) {
