...
class Slots
{
    ...
    public function __construct( AvailableTime $at, Entities\Service $service, $number_of_persons, $extras_duration )
    {
        ...
        if ( Config::isServiceScheduleEnabled() && $service->get( 'start_time' ) !== null ) {
            $this->service_breaks = apply_filters( 'bookly_service_schedule_get_breaks', array(), $service->get( 'id' ) );
            $this->time_from = max( $this->time_from, Utils\DateTime::timeToSeconds( $service->get( 'start_time' ) ) );
            $this->time_to   = min( $this->time_to,   Utils\DateTime::timeToSeconds( $service->get( 'end_time' ) ) );
        }
    }
        ...
        foreach ( $this->at->staff_data as $staff_id => $staff ) {
            ...
            if ( $schedule ) {
                if ( $this->is_all_day_service ) {
                    ...
                    $intersections = $this->_findIntersections( $schedule['start_time'], $schedule['end_time'], $this->time_from, $this->time_to );
                }
                ...
                foreach ( $intersections as $intersection ) {
                    if ( $intersection['end'] - $intersection['start'] >= $this->service->get( 'duration' ) ) {
                        ...
                        if ( ! $this->is_all_day_service ) {
                            ...
                            foreach ( array_merge( $schedule['breaks'], $this->service_breaks ) as $break ) {
                                $frames = $this->_removeTimePeriod( $frames, $break['start'], $break['end'] );
                            }
                        }
