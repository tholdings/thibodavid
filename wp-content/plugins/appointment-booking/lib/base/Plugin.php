    ...
    public static function registerHooks()
    {
        ...
        if ( is_admin() ) {
            ...
            add_action( 'admin_notices',  function () use ( $plugin_class ) {
                    ...
                    printf(
                        ...
                        <script type="text/javascript">
                            jQuery("a#%1$s_notice_dismiss").click(function (e) {
                                ...
                                jQuery.ajax({
                                    url: "%6$s",
                                    data: {action: "bookly_dismiss_admin_notice", prefix: "%1$s"}
                                });
                            });
                        </script>',
                ...
                if ( $plugin_class == 'Bookly\Lib\Plugin' && get_option( 'bookly_gen_show_subscribe_notice' ) && isset( $_REQUEST['page'] ) && substr( $_REQUEST['page'], 0, 7 ) == 'bookly-' ) {
                    printf( '<div id="bookly-tbs" class="wrap"><div class="alert alert-info bookly-mail-request"><button type="button" class="close" data-dismiss="alert"></button> <i class="alert-icon"></i><p class="alert-title">%s</p><div class="alert-body"><div class="input-group input-group-sm" style="max-width: 400px"><span class="input-group-addon"><i class="glyphicon glyphicon-envelope"></i></span><input type="text" class="form-control" id="bookly-email-subscribe"><span class="input-group-btn"><button class="btn btn-info" type="button">%s</button></span></div><p class="alert-help-block">%s</p></div></div></div><script type="text/javascript">jQuery(function($){$(\'.bookly-mail-request button\').on(\'click\',function(){$.post(ajaxurl,{action: \'bookly_set_subscription_options\', email: $(this).hasClass(\'close\') ? undefined : $(\'#bookly-email-subscribe\').val() }, function(response){if(response.success){$(\'.bookly-mail-request\').remove();}else{alert(response.data.message);}});});});</script>',
                        __( 'We’re excited to announce major updates soon. Leave us your email to stay in the loop!', 'bookly' ),
                        __( 'Send',  'bookly' ),
                        __( 'Promise we won’t email you more than once a month.', 'bookly' )
                    );
                }
            }, 10, 0 );
    ...
    protected static function initUpdateChecker()
    {
        if ( get_option( static::getPurchaseCode() ) ) {
            ...
            add_filter( 'puc_request_info_result-' . static::getSlug(), function ( $pluginInfo, $result ) {
                if ( ! $result instanceof \WP_Error && isset( $result['body'] ) ) {
