...
class PayuLatam
{
    ...
    public static function renderForm( $form_id, $response_url )
    {
        ...
        $replacement['%responseUrl%']     = $response_url . 'action=ab-payu_latam-checkout&ab_fid=' . $form_id;
        $replacement['%confirmationUrl%'] = $response_url . 'action=ab-payu_latam-ipn&ab_fid=' . $form_id;
        ...
        if ( ! empty( $replacement ) ) {
            $form = '<form action="%action%" method="post" class="ab-%gateway%-form" data-gateway="%gateway%">
                ...
                <input type="hidden" name="extra1" value="" class="bookly--pending_appointments">
