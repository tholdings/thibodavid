...
class Controller extends Lib\Base\Controller
{
    ...
    public function executeRenderCart()
    {
        ...
        if ( $userData->load() ) {
            ...
            $this->info_text = $this->_prepareInfoText( 4, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_cart_step' ), $userData );
            $cart_items = array();
            $use_client_time_zone = get_option( 'bookly_gen_use_client_time_zone' );
    ...
    public function executeRenderDetails()
    {
        ...
        if ( $userData->load() ) {
            ...
            $this->info_text = $this->_prepareInfoText( 5, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_details_step' ), $userData );
    ...
    public function executeRenderPayment()
    {
        ...
        if ( $userData->load() ) {
            ...
            if ( $payment_disabled == false ) {
                ...
                $this->info_text = $this->_prepareInfoText( 6, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_payment_step' ), $userData );
                $this->info_text_coupon = $this->_prepareInfoText( 6, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_coupon' ), $userData );
                ...
                $response = array(
                    ...
                    'html'     => $this->render( '6_payment', array(
                        ...
                        'pay_payu_latam'    => ! Lib\Config::isPaymentDisabled( Lib\Entities\Payment::TYPE_PAYULATAM ),
                        ...
                        'response_url'     => $this->getParameter( 'response_url' )
                    ), false )
                );
            } else {
                $response = array(
    ...
    public function executeRenderComplete()
    {
        ...
        if ( $userData->load() ) {
            ...
            do {
                ...
                $this->info_text = $this->_prepareInfoText( 7, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_complete_step' ), $userData );
            } while ( 0 );
    ...
    public function executeSavePendingAppointment()
    {
        ...
        if ( in_array( $gateway, $valid_gateways ) && get_option( 'bookly_pmt_' . $gateway ) ) {
            $userData = new Lib\UserBookingData( $this->getParameter( 'form_id' ) );
            if ( $userData->load() ) {
                $failed_cart_key = $userData->cart->getFailedKey();
                if ( $failed_cart_key === null ) {
        ...
        if ( $userData->load() ) {
            ...
            if ( $coupon->isLoaded() && $coupon->get( 'used' ) < $coupon->get( 'usage_limit' ) ) {
                ...
                if ( $coupon->valid( $service_ids ) ) {
                    ...
                    $response = array(
                        'success' => true,
                        'text'    => $this->_prepareInfoText( 6, Lib\Utils\Common::getTranslatedOption( 'bookly_l10n_info_coupon' ), $userData ),
                        'total'   => $total
                    );
                } else {
