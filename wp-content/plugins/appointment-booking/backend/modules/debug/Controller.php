...
class Controller extends Lib\Base\Controller
{
    ...
    public function executeExportData()
    {
        ...
        foreach ( apply_filters( 'bookly_plugins', array() ) as $plugin ) {
            ...
            foreach ( array( 'data_loaded', 'installation_time', 'db_version' ) as $option ) {
                $option_name = $plugin_prefix . $option;
                $result['options'][ $option_name ] = get_option( $option_name );
            }
            $result['options'][ $plugin::getPurchaseCode() ] = get_option( $plugin::getPurchaseCode() );
    ...
    public function executeImportData()
    {
        ...
        if ( $file = $_FILES['import']['name'] ) {
            ...
            if ( $json !== false) {
                ...
                header( 'Location: ' . admin_url( 'admin.php?page=ab-debug&status=imported' ) );
            }
        }

        header( 'Location: ' . admin_url( 'admin.php?page=ab-debug' ) );
