...
<form method="post" action="<?php echo esc_url( add_query_arg( 'tab', 'google_calendar' ) ) ?>" class="ab-settings-form">
    ...
    <div class="form-group">
        <?php \Bookly\Lib\Utils\Common::optionText( __( 'Client ID', 'bookly' ), 'bookly_gc_client_id', __( 'The client ID obtained from the Developers Console', 'bookly' ) ) ?>
    </div>
    <div class="form-group">
        <?php \Bookly\Lib\Utils\Common::optionText( __( 'Client secret', 'bookly' ), 'bookly_gc_client_secret', __( 'The client secret obtained from the Developers Console', 'bookly' ) ) ?>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label for="bookly_gc_two_way_sync"><?php _e( '2 way sync', 'bookly' ) ?></label>
        ...
        <?php \Bookly\Lib\Utils\Common::optionToggle( 'bookly_gc_two_way_sync', array( array( '0', __( 'Disabled', 'bookly' ) ), array( '1', __( 'Enabled', 'bookly' ) ) ) ) ?>
    </div>
    <div class="form-group">
        <label for="bookly_gc_limit_events"><?php _e( 'Limit number of fetched events', 'bookly' ) ?></label>
        ...
        <select id="bookly_gc_limit_events" class="form-control" name="bookly_gc_limit_events">
            <?php foreach ( array( __( 'Disabled', 'bookly' ) => '0', 25 => 25, 50 => 50, 100 => 100, 250 => 250, 500 => 500, 1000 => 1000, 2500 => 2500 ) as $text => $limit ) : ?>
                <option value="<?php echo $limit ?>" <?php selected( get_option( 'bookly_gc_limit_events' ), $limit ) ?> ><?php echo $text ?></option>
            <?php endforeach ?>
        </select>
    </div>
    <div class="form-group">
        <?php \Bookly\Lib\Utils\Common::optionText( __( 'Template for event title', 'bookly' ), 'bookly_gc_event_title', __( 'Configure what information should be placed in the title of Google Calendar event. Available codes are {service_name}, {staff_name} and {client_names}.', 'bookly' ) ) ?>
    </div>
